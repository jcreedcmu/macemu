/*	1.01 - kTopLeft - This is for positioning the Disk Initialization
 * dialogs. */

#define kDITop 0x0050
#define kDILeft 0x0070

/* 1.01 - changed constants to begin with 'k' for consistency, except for
 * resource IDs */
/*	kTextMargin is the number of pixels we leave blank at the edge of the
 * window. */
...
/*	kMaxTELength is an arbitrary number used to limit the length of text in
   the TERec so that various errors won't occur from too many characters in the
   text. */
...
/* kOSEvent is the event number of the suspend/resume and mouse-moved events
   sent by MultiFinder. Once we determine that an event is an OSEvent, we look
   at the high byte of the message sent to determine which kind it is. To
   differentiate suspend and resume events we check the resumeMask bit. */
